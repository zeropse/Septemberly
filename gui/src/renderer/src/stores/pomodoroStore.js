...
export const usePomodoroStore = create(
  persist(
    (set, get) => ({
      ...
      switchMode: (newMode) => {
        const nextSeconds = newMode === 'focus' ? POMODORO.FOCUS : POMODORO.BREAK
        set({
          mode: newMode,
      ...
      tick: () => {
        ...
        const now = Date.now()
        const elapsed = state.lastUpdated ? Math.floor((now - state.lastUpdated) / 1000) : 1
      ...
      initializeFromStorage: () => {
        ...
        if (state.running && state.lastUpdated) {
          ...
          if (elapsedSeconds > 0) {
            const newSecondsLeft = Math.max(0, state.secondsLeft - elapsedSeconds)
