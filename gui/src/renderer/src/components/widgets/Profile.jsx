...
export default function ProfileCard() {
  ...
  const getCurrentLevelInfo = useGamificationStore((state) => state.getCurrentLevelInfo)
  const getProgressToNextLevel = useGamificationStore((state) => state.getProgressToNextLevel)
    ...
    <Card className="min-w-sm max-w-lg">
      <CardHeader className="flex flex-row items-center gap-4">
        ...
        <div className="flex flex-col gap-2 flex-1">
          ...
          <div className="flex gap-6">
            <Badge>
              {trait === 'none' ? 'None' : trait.charAt(0).toUpperCase() + trait.slice(1)}
            </Badge>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-center space-y-2">
          <div className="text-lg font-medium text-chart-3">{currentLevel.name}</div>
          <div className="text-sm opacity-80">
            Total XP:
            <span className="font-bold text-green-600 dark:text-green-400">{totalXP}</span>
          </div>
        </div>
        ...
        {!isMaxLevel && (
          <div className="space-y-2">
            ...
            <div className="text-xs text-center opacity-70">{remaining} XP to next level</div>
          </div>
        )}

        {isMaxLevel && (
          <div className="text-center text-chart-3 font-bold">Maximum Level Reached!</div>
        )}
        ...
        {recentActions.length > 0 && (
          <div className="space-y-2">
            <div className="text-sm font-medium text-center my-3">Recent Activities</div>
              ...
              {recentActions.slice(0, 3).map((action) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 ml-2">
                    <span className="text-green-600 dark:text-green-400">+{action.xpGained}</span>
                    {action.levelUp && (
