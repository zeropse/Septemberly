...
export default function Quote({ className }) {
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle className="font-semibold text-center text-3xl">Quote of the Day</CardTitle>
      </CardHeader>
      ...
      <CardContent className="mb-10 flex flex-col justify-between min-h-[100px]">
        <div>
          ...
          <div className="text-right mt-2 text-sm text-gray-300">— {quote.author}</div>
        </div>
      </CardContent>

      <CardFooter>
        <Button size="sm" onClick={handleRefresh} className="w-full cursor-pointer">
          Refresh
        </Button>
      </CardFooter>
    </Card>
